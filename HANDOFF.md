...
- **Task state transitions go through `internal/orchestrator/lifecycle.Coordinator`, not `Store` directly.** Dispatch/monitor/recovery callers invoke domain verbs (`Assign`, `Start`, `Complete`, `Requeue`, `Cancel`, `Recover`, `MarkRecovering`, `MarkReadyForRetry`, `FailDispatch`) and the coordinator picks the Store method, pairs the slot-accounting update, and emits the matching `task.*` event. New transitions should add a coordinator method rather than calling `Store.AssignTask`/`StartTask`/`CompleteTask`/`RequeueTask`/`UpdateTaskStatus` from orchestrator code.
- **Requeue (both monitor and recovery paths) now releases the instance slot and emits an event.** Previously the monitor's `requeueTask` and recovery's `requeueRecoveringTask` only decremented the local counter, silently leaking `instances.running_containers`; and both returned tasks to `pending` without a webhook event. `Coordinator.Requeue(RequeueInterrupted|RequeueRecovering)` now pairs the DB write with `Slots.Release` (local counter + instance slot) and emits `task.interrupted` or `task.recovering`. Webhook consumers on the recovery path now see an extra `task.recovering` event per requeue in addition to the initial startup mark.
- **`api.CancelTask` still drives the REST cancel path; it is not routed through the coordinator.** `Coordinator.Cancel` exists and is the correct entry point for orchestrator-side cancellation, but the HTTP handler layer continues to call the existing `api.CancelTask` helper + emit `task.cancelled` directly. Wiring the API handler stack through the coordinator would require threading the orchestrator's coordinator into `NewServer`; deferred until there is a concrete reason (e.g. handler-side slot accounting).
